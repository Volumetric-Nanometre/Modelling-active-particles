...
void initialVelocities(int numberOfParticles, particleVariables *particles, environmentVariables *conditions, time_t tSeed)
{
	...
	if (gDebug) printf("Total energy of each particle is %lf J",totalEnergy);

	// kT = (1/2)mv^2 + (1/2)Iw^2
	// => v^2 = kT/m  && w^2 = kT/I		- split kinetic energy equally between rotational and translational motion

	double vSquared = totalEnergy/conditions->mass;
	double wSquared = totalEnergy/( (2/5) * conditions->mass * pow(conditions->radius,2) ); // Moment of inertia of a solid sphere is (2/5)mr^2
	...
	for (int i = 0; i < numberOfParticles; i++)
	{
		...
		particles[i].dgamma = randSign(tSeed) * sqrt( wSquared - pow(particles[i].dalpha,2) - pow(particles[i].dgamma,2) ); // calculate w_z^2 so that w_x^2 + w_y^2 + w_z^2 = w^2*/
